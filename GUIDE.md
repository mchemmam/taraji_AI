...
1. **Collect** — pulls articles from Google News (`collectors/google_news.py`) and a curated list of RSS feeds (`collectors/rss_collector.py`, config in `config/rss_feeds.json` — currently BBC Sport, L'Équipe, Foot Mercato, Babnet, Mosaïque FM, Alchourouk, CAF Online, Tunisie Numérique, Jeune Afrique).
...
python main.py classify           # re-classify existing articles in the DB
```
